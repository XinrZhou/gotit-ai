...
**gotit-ai** 是**个人单用户**的**技术成长系统**：日常学习搭子在 thread 里陪你聊、跨会话记住薄弱点，到该证明时跑验证工作流。只有证据过关才算会——掌握门禁是**确定性代码**，不是 LLM 随口说一声。不是多租户产品，也不是又一个只负责存更多的笔记库。

## 它能做什么

| 能力 | 含义 |
|------|------|
| **搭子对话** | Thread、@mention、带记忆的人格回复；注入当日计划摘要，问「今日计划」有据可依；白名单工具查欠账/备开考/备深挖/读教训（气泡下安静轨迹；可一键「开考」「深挖」） |
| **A2A 接力** | Agent 可在同一轮把球转给同伴（ball custody） |
| **工作流** | 考我 / 回讲 / 项目深挖——从聊天主面发起；回合可写入同一 thread。**过门**仅考我与（绑定 claim 的）回讲；深挖是项目练习，不算掌握 |
| **今日简报** | 空对话展示欠练 + 今日计划，一键开考；可收工；临近面试时安静建议深挖 |
| **冷启动校准** | 少题数摸底，初始化排程与易混图；无欠账但有 claim 时出现「先摸底一下」 |
| **第一次过关** | 空库引导：笔记 → claim → 开考/摸底 → 看到门禁结果 |
| **验证闭环** | 考察 → Critic 复核 → 确定性 gate → 轨迹 / 间隔复习（按失败次数拉长间隔；due 排序 + 易混/前置依赖） / 掌握图谱写回；对话内可点欠账/结果块 |
| **笔记 → claim** | `POST /v1/notes/{id}/ingest`（Compass / stub）→ 可检验主张 + 今日计划；动态「有用」可晋升为可考主张 |
| **回讲** | 文本或口说转写；与考我同一套终审 |
| **简历深挖** | 项目 / 简历驱动的模拟面试（桑迪） |
| **设置** | 我 / 提醒 / 高级 — 资料与临近备考提醒、计划提醒、Skills+MCP；顶栏「动态」观测 OpenClaw 写回 |
| **弱点图谱** | 对话顶栏「弱点图谱」→ 主栏内嵌易混/薄弱/前置依赖力导向图（保留左侧对话） |
...
gotit-api（FastAPI）  ←→  Postgres / SQLite
        │                   ▲
        │  共用 db.ops      │
gotit-mcp（stdio） ─────────┘   ← OpenClaw / 宿主 Agent

无进程内 Worker。早晚报 / 提醒 cron 在 OpenClaw
（Gateway + skills/*）；gotit 存 prefs 并提供 sync 钩子。
```

**设计原则：** 搭子拥有聊天面。**验证是核心闭环**（掌握标准只认过关），不是无头流水线。OpenClaw 是可选渠道。部署定位是**个人单用户**（`GOTIT_USER_ID` + API key），不做多租户 SaaS。
...
docker compose up -d postgres   # 或用 .env 里的 SQLite
...
**过一遍门，才算真会了。** 聊天是表面，验证是核心闭环；掌握标准只认过关。

1. **聊** — 开 thread，@ 搭子，可选技能  
2. **导入** — 加笔记，「出题」→ note ingest（Compass）→ claim + 今日计划  
3. **验证** — 从聊天发起考我 / 回讲（过门）；项目深挖是练习场，不算掌握  
4. **门禁** — 凯伦复核；代码判定过了 / 还差点 / 欠着下次  
5. **记住** — 结果写入轨迹，下次更有针对性  
...
| 一键深挖（open_drill / 面试 upcoming） | 完成 |
| **主路径收敛**（打开 → 过一关更顺；机制藏后台） | **进行中** |
| 深挖 ↔ 掌握叙事说清（prep-only，或日后接同一终审） | 下一步 |
| 完整 APPLY 验证形态（今日降级为 probe） | 以后 |
| 按 agent 绑不同模型（Critic 已可单独配） | 以后 |
| Agent 挂载完整 MCP 目录进聊天 | **不做近期待办**（保持白名单） |
...
| P1 | 验证过 = 会了 | 自信不是证据 |
| P2 | 失败有用 | 未过 → 小课 + 再检 + 轨迹 |
| P3 | 形式服从知识点 | claim 偏好 → 开考/回讲（过门）；深挖为项目练习，默认不算掌握 |
